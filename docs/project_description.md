...
- v0.5: three new agentic_tools (Cursor, Gemini CLI, GitHub Copilot) and three new customization_types (`rules`, `slash_command`, `mcp_server`). Antigravity remains the canonical owner of `~/.gemini/antigravity/skills/`; Gemini CLI also exposes `~/.gemini/skills/` as a normal `skill` directory. A new top-level config key `secret_policy` (`secrets_refused` / `secrets_accepted`, default `secrets_refused`) governs how literal secrets in MCP-server `env`, `headers`, and `auth.*` fields are handled. The deprecated `mcp_server_secret_policy` key and its legacy values are accepted for one release as compatibility aliases, scheduled for removal in v0.6 (see `docs/mcp_server_secret_policy_deprecation_cleanup_plan.md`).
...
- Graceful absence: if a registered, enabled agentic_tool's directory becomes missing or unreadable, the tool is marked `unavailable` and the transition is logged once. An `unavailable` tool never causes a removal to propagate to healthy tools (US-11).
- Explicit generated filenames for new counterparts; agents and skills use the bare slugified artifact name inside their distinct directories (e.g. `formatter/SKILL.md` for skills).
...
Per-customization_artifact state is stored under the platform default state directory (Linux `~/.local/state/agents-sync/`; Windows `%LOCALAPPDATA%\\agents-sync\\state\\`):

- `state.json` — versioned envelope: `{"schema_version": 4, "customization_artifacts": {<customization_artifact_id>: {"customization_type": ..., "canonical_digest": ..., "agentic_tools": {<name>: {"path": ..., "digests": ...}}}}}`.
...
- **`agentic_tool` status** (prose: "available / unavailable / disabled") — at a given poll: `available` (configured, enabled, directory reachable), `unavailable` (configured, enabled, directory missing or unreadable), or `disabled` (turned off in config).
...
- **Available `agentic_tools` at a given poll** — registered, enabled agentic_tools whose status is `available` (directory reachable, readable, writable).
...
- **Archive** — directory under the state directory where prior versions of files are preserved before any destructive overwrite.
...
- **Slug** — the filesystem-friendly form of a customization_artifact's `name`; determines the basename of a rendered file. Agents and skills live in separate directories, so generated counterparts use the bare slug (e.g. `formatter/SKILL.md`).
